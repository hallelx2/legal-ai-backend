...
export class DocusignService {
  ...
  private readonly AUTH_SERVER = process.env.DOCUSIGN_AUTH_SERVER || 'account-d.docusign.com';
  ...
  async createToken(CodeDto:CodeDto) {
    ...
    try {
      ...
      console.log(data)
      return await this.saveTokens(CodeDto.user_id,
        data.access_token,
        data.refresh_token,
        data.expires_in)
    } catch (error) {
  ...
  async refreshAccessToken(CodeDto:CodeDto): Promise<AuthToken | null> {
    const existingToken = await this.authTokenModel.findOne({ refreshToken: CodeDto.code });
    ...
    // This is a placeholder for that logic
     ...
     if(!data) return null;
     return this.saveTokens(CodeDto.user_id,
      data.access_token,
      data.refresh_token,
      data.expires_in)
  }
